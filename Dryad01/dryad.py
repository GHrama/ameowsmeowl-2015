...
def main():
    ...
    # DB_HOST
    db_host = configFile.get("middleware_props", "dbservername")
    ...
    middleware_set = set([x[1] for x in configFile.items("mapping")])
    client_set = set([x[0] for x in configFile.items("mapping")])

    # make a map of elements such that middleware[client]- middleware

    print middleware_set
    print client_set

    # can get middleware hostname from client hostname
    mw_cl_map = {}
    for cl, mw in configFile.items("mapping"):
        mw_cl_map[cl] = mw
    ...
    database_port = configFile.get("middleware_props", "dbport")
