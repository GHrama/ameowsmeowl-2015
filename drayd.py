...
MIDDLEWARE_PROP_KEYS = {
        'database' : ['db_name', 'db_port', 'db_username', 'db_password', 'db_connection_limit'], # Excludes db_url. This is auto added by reusing hostname.
        'middleware' : ['pool_size', 'middleware_port'],
        }

CLIENT_PROP_KEYS = {
        'middleware' : ['middleware_port'], # Excludes mw_url. This is auto-added by reusing hostname.
        }
...
def main():
    ...
    parser.add_argument("experimentlist", help="the expeiment file path")
    ...
    copy_file_to_host(username,
                      mw_host,
                      None,
                      configFile.get("paths","middleware.jar"),
                      "/local/rsridhar/%s/middleware.jar" % experiment_id)
    ...
    with open(prop_file_on_local_machine_S,'w') as f:
        ...
        for keys in configFile.options("middleware_props"):
          f.write("%s=%s"%(keys,configFile.get("middleware_options",keys)))
    ...
    with open(prop_file_on_local_machine_C,"w") as fc:
      ...
      for keys in configFile.options("client_props"):
                fc.write("%s=%s"%(keys,configFile.get("client_options",keys)))
    ...
    run_remote_cmd(username,
                   db_host,
                   None,
                   "screen -S dstat -X stuff 'dstat -Tnmc --output /tmp/dstat-db-%s.log'`echo -ne '\015'`"%experiment_id
                   )
    
    
    #######################
    #start middleware
    middle_jar_path = "/local/rsridhar/%s/middleware.jar"%experiment_id
    # usually no command arguments from middleware
    cmd_args=''
    for option in configFile.options('middleware_cl_args'):
      cmd_args += '--%s %s ' % (option, configFile.get('middleware_cl_args', option))
    ...
    for option in configFile.options("client_cl_args"):
        cmd_args += '--%s %s' % (option,configFile.get("client_cl_args",option))

    print 'Client machines :: ',cl_host
    for(machine_num,cl_host) in enumerate(cl_host):
        cmd_args_this_host = cmd_args + "--machine_id %d" %machine_num
        client_cmd = "%s -jat client.jar %s"%(java_path_dryad,cmd_args_this_host)
    ...
    run_remote_cmd(username,
              db_host,
              None,
              "/local/rsridhar/pgsql/bin/pg_dump message -f /tmp/%s.sql" % (experiment_id),
              )
    # get it locally
    ...
    for (mw_num, mw_host) in enumerate(mw_host):
        with warn_only():
          run_remote_cmd(username,
                  mw_host,
                  None,
          ...
          get_file_from_host(username,
                  ...
                  "%s/middleware-logs-%s.tgz" % (directory, mw_num),
                  )
    with warn_only():
        #for each clien do:
      for (machine_num, cl_host) in enumerate(cl_host):
