...
1. Python script dryad_easy.py, it allows for multiple
   experiments by changing important configurations such as duration,server,clients,database in only the first file.
   (Since focus of this course was not to make perfect scripts, scripts might be buggy but generally runs very well!
   if it happens that something goes wrong,please try to run the script once again!)
2. Dryad01.ini is an example configuration file, most parameters are well tuned, what you can vary is :
	1. duration
	2. server
	3. client
	4. server
	5. And the number of each server and client machines/threads, the configuration file must be self explanatory,
...
3. dbFiles contains the
	...
	4. Nothing needs to be changed here.
	



--INSTRUCTIONS- How to run Ganesha System:
...
4. Experiment_list_ec2.txt contains the Dryad01.ini configuration file location, which you can modify of course,
   it is right now set to run on the dryad. In this way multiple file experiments can be run one after the other!
   But you have to create more configuration fiels for this!
5. All experiment outputs will be in output folder, all zipped with the experiment id.
...
1. The plot scripts are made to accept many file directories, each containing
	1 repetition of the same experiment for different factor values.
