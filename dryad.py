...
def main():
    ...
    java_path_dryad = config.get("constants", "dryad_java7_path")
    pem_key_path = None
    ...
    '''if masterconfig.get("setup", "is_ec2") in ["True", "true"]: # If host is on EC2 install gcc, etc.
        for mw_host in middleware_set:
            install_basic_utils(masterconfig.get("setup", "username"), # ec2-user or torekond
                  mw_host,
                  masterconfig.get("paths", "pem_key_path"),
                  )
            setup_java_env(masterconfig.get("setup", "username"), # ec2-user or torekond
                  mw_host,
                  masterconfig.get("paths", "pem_key_path"),
                  )
    ...
    for experiment in exp_list:
        ...
        mw_cl_map = {}
        for mw, cl in configFile.items("mapping"):
            item_list = cl.split(',')
