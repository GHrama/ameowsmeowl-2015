...
public class MessageServer implements Runnable {
	...
	public MessageServer(ExecutorService messageWorkerPool,
			 String host, int port, int bufferSize, PGPoolingDataSource pool)
			throws IOException {
		this.host = host;
		this.port = port;
		this.running = true;
...
//			int nothreads = 10;
			...
			dbConnectionPool.setDataSourceName("meow");
		dbConnectionPool.setServerName(sn);
		dbConnectionPool.setDatabaseName(dn);
		dbConnectionPool.setPortNumber(port);
...
//			}
	    ...
	    servicePool.execute(new MessageServer(servicePool,host, port1, ms,dbConnectionPool));
